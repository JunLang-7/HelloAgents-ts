...
The TypeScript API is idiomatic camelCase. Its _serialized_ payloads preserve
...
symbols from `@junlang-7/helloagents`.
...
The target TypeScript layout follows Python's package hierarchy one-to-one
under `hello_agents/`, while using lower-case kebab-case TypeScript file names.

| Python source                                                                                      | Target TypeScript module(s)                                                                              | Status / owning issue |
| -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------- |
| `core/config.py`, `message.py`, `exceptions.py`, `llm_response.py`, `lifecycle.py`, `streaming.py` | `hello_agents/core/config.ts`, `message.ts`, `errors.ts`, `responses.ts`, `lifecycle.ts`, `streaming.ts` | #1                    |
| `core/llm.py`, `llm_adapters.py`                                                                   | `hello_agents/core/llm.ts`, `hello_agents/adapters/{base,openai,anthropic,gemini}.ts`                    | #2, #16               |
| `tools/response.py`, `errors.py`, `base.py`, `registry.py`                                         | `hello_agents/tools/{response,errors,tool,registry}.ts`                                                  | #6                    |
| `tools/circuit_breaker.py`                                                                         | `hello_agents/tools/circuit-breaker.ts`                                                                  | #5                    |
| `context/{history,token_counter,truncator,builder}.py`                                             | `hello_agents/context/{history,token-counter,truncator,builder}.ts`                                      | #7                    |
| `core/agent.py`, `session_store.py`                                                                | `hello_agents/core/{agent,session-store}.ts`                                                             | #11                   |
| `agents/{simple_agent,react_agent,reflection_agent,plan_solve_agent,factory}.py`                   | `hello_agents/agents/{simple-agent,react-agent,reflection-agent,plan-solve-agent,factory}.ts`            | #8, #10, #13, #44     |
| `observability/trace_logger.py`                                                                    | `hello_agents/observability/trace-logger.ts`                                                             | #14                   |
| `tools/builtin/{calculator,file_tools}.py`                                                         | `hello_agents/tools/builtin/{calculator,file-tools}.ts`                                                  | #12                   |
| `skills/loader.py`, `tools/builtin/skill_tool.py`                                                  | `hello_agents/skills/loader.ts`, `hello_agents/tools/builtin/skill-tool.ts`                              | #17                   |
| `tools/tool_filter.py`, `tools/builtin/task_tool.py`                                               | `hello_agents/tools/{tool-filter.ts,builtin/task-tool.ts}`                                               | #15                   |
| `tools/builtin/{todowrite_tool,devlog_tool}.py`                                                    | `hello_agents/tools/builtin/{todo-write-tool,dev-log-tool}.ts`                                           | #18                   |
| Python tests                                                                                       | `test/**/*.test.ts` plus shared fixture data                                                             | #19                   |
| package `__init__.py` modules                                                                      | `hello_agents/index.ts` and explicit subpath exports                                                     | #4, #19               |
...
| Circuit-breaker recovery    | A timed-out open circuit immediately returns to closed on its next status check.                                           | Same timed-out automatic recovery behavior.                                                                    | Expose the recovered interval as `half-open` and permit exactly one probe. Its success closes the circuit; its error reopens it. This preserves upstream recovery results while preventing concurrent retry bursts.                  |
...
| Agent factory and subagents | Python/Go expose `agents.factory` while child execution is coordinated by the core agent and `TaskTool`.                   | Go keeps the factory and task tool as separate packages.                                                       | `hello_agents/agents/factory.ts` owns `createAgent` plus the isolated child-agent implementation used by `TaskTool`; this keeps the public factory mapped to the upstream module while avoiding an unmapped `subagent.ts` extension. |
| Source root                 | Python and Go use a top-level `hello_agents/` package directory.                                                           | TypeScript commonly uses `src/`, but that does not provide structural parity.                                  | Use `hello_agents/` as the TypeScript source root. Its package hierarchy aligns one-to-one with both upstream repositories; build output and public package exports remain `dist/`.                                                  |
