...
if (!enabled || missing.length > 0) {
  process.stdout.write(
    'Skipping real API integration test (opt-in and required credentials are not configured).\n'
  );
  process.exit(0);
}
...
try {
  ...
  const response = await llm.invoke(
    [
      {
        role: 'user',
        content: 'Reply with a short greeting for the HelloAgents integration check.'
      }
    ],
