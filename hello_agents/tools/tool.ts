...
/** `tool_action` 元数据；TypeScript 没有 Python 装饰器反射，因此由工具工厂读取。 */
...
export abstract class Tool<TSchema extends z.ZodType = z.ZodType> {
  ...
  /** 返回兼容 Python `get_parameters()` 的参数元数据副本。 */
  public getParameters(): ToolParameter[] {
    return this.parameters.map((parameter) => ({ ...parameter }));
  }

  /** 返回兼容 Python `to_dict()` 的工具描述。 */
  public toDict(): { name: string; description: string; parameters: ToolParameter[] } {
...
export function toolAction<TSchema extends z.ZodType>(
  ...
  return ((target: (...args: never[]) => unknown) => {
    const decorated = target as typeof target & ToolActionMetadata;
...
export class AutoGeneratedTool<TSchema extends z.ZodType = z.ZodType> extends Tool<TSchema> {
  ...
  public constructor(options: {
    ...
    const name = options.name ?? `${options.parent.name}_${options.method.name.replace(/^_+/, '')}`;
    super({
      name,
      description: options.description ?? `执行 ${options.method.name}`,
      inputSchema: options.inputSchema,
      ...(options.parameters === undefined ? {} : { parameters: options.parameters })
    });
    this.handler = options.method;
  }
