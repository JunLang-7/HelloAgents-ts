...
| `agents/{simple_agent,react_agent,reflection_agent,plan_solve_agent,factory}.py`                   | `src/agents/{simple-agent,react-agent,reflection-agent,plan-solve-agent,factory}.ts`            | #8, #10, #13, #44     |
...
| Agent factory and subagents | Python/Go expose `agents.factory` while child execution is coordinated by the core agent and `TaskTool`.                   | Go keeps the factory and task tool as separate packages.                                                       | `src/agents/factory.ts` owns `createAgent` plus the isolated child-agent implementation used by `TaskTool`; this keeps the public factory mapped to the upstream module while avoiding an unmapped `subagent.ts` extension. |
| Source root                 | Python and Go use a top-level `hello_agents/` package directory.                                                           | TypeScript package conventions commonly use `src/` as the source root.                                         | Keep `src/` as the TypeScript source root. The module map already defines `src/...` targets, and renaming would create repository-wide path churn without changing the public `dist` package layout or runtime behavior.    |
